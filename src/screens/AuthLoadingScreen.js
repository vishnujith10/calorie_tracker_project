...
const AuthLoadingScreen = ({ navigation }) => {
  useEffect(() => {
    ...
    const checkAuthAndOnboarding = async () => {
      try {
        // ─── DEV MODE: Jump directly to a specific screen on every reload ───
        // Set EXPO_PUBLIC_DEV_SCREEN in .env to the screen name you're working on.
        // This skips all auth/onboarding logic so Fast Refresh lands you instantly.
        // Leave empty or remove for normal auth flow in production.
        const devScreen = process.env.EXPO_PUBLIC_DEV_SCREEN;
        if (__DEV__ && devScreen && devScreen.trim() !== '') {
          console.log(`🛠️  [DEV MODE] Jumping directly to screen: "${devScreen.trim()}"`);
          navigation.reset({
            index: 1,
            routes: [
              { name: 'Welcome' },       // back stack so the back button works
              { name: devScreen.trim() },
            ],
          });
          return;
