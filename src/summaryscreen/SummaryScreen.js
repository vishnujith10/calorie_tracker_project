...
const SummaryScreen = () => {
  ...
  const generateAISummary = useCallback(async (data) => {
    ...
    for (const modelName of models) {
      try {
        ...
        console.warn(`SummaryScreen - ${modelName} failed:`, err?.message);
        lastError = err;
      }
    }
  ...
  const generateReportText = useCallback(() => {
    if (!aiData || !rawData) return '';
    const calPct = rawData.calorieGoal > 0 ? Math.round((rawData.calories / rawData.calorieGoal) * 100) : 0;
    const hydPct = rawData.hydrationGoal > 0 ? Math.round((rawData.hydration / rawData.hydrationGoal) * 100) : 0;

    let tipsFormatted = '';
    if (aiData.tips && aiData.tips.length > 0) {
      tipsFormatted = '\n--------------------------------------------------\n4. ACTIONABLE TIPS FOR TODAY\n--------------------------------------------------\n' +
        aiData.tips.map((t, i) => `[${i + 1}] ${t.title.toUpperCase()}\n    ${t.description}`).join('\n\n');
    }

    return `==================================================
              CALORA MINDFUL DIARY
...
--------------------------------------------------
Overall Health Score : ${aiData.overallScore}/100
Status               : ${aiData.statusBadge || 'On Track'}
Summary              : ${aiData.executiveSummary || 'No summary available.'}

--------------------------------------------------
2. NUTRITION & DAILY METRICS
--------------------------------------------------
• Calories Intake : ${rawData.calories} / ${rawData.calorieGoal} kcal (${calPct}%)
• Hydration       : ${rawData.hydration}L / ${rawData.hydrationGoal}L (${hydPct}%)
• Sleep Duration  : ${formatDuration(rawData.sleepMinutes)}
• Log Streak      : ${rawData.streak} Days 🔥

--------------------------------------------------
3. AI HEALTH ASSESSMENT
--------------------------------------------------
Pace Assessment : ${aiData.paceAssessment?.headline || 'Balanced Pace'}
Assessment Note : ${aiData.paceAssessment?.explanation || 'No detailed assessment provided.'}
${tipsFormatted}

==================================================
     Generated by Calora AI • Mindful Tracker
==================================================`;
  }, [aiData, rawData, yesterdayDisplay]);

  // ── Download Report File Handler ──
  const handleDownload = async () => {
    if (!aiData || !rawData) {
      Alert.alert('Notice', 'Summary data is still loading.');
      return;
    }
    ...
    try {
      ...
      const fileName = `Calora_Summary_${yesterdayDate}.txt`;
      const fileUri = `${FileSystem.documentDirectory}${fileName}`;

      // Write text report to file system
      await FileSystem.writeAsStringAsync(fileUri, reportText);

      // Share complete formatted summary report text
      await Share.share({
        title: `Calora Summary Report (${yesterdayDisplay})`,
        message: reportText,
      });
    } catch (error) {
      console.error('Download summary error:', error);
      Alert.alert('Download Error', 'Could not generate report file. Please try again.');
    } finally {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        bounces={true}
      >
        <View ref={viewShotRef} collapsable={false} style={{ backgroundColor: palette.background, paddingBottom: 16 }}>
        {/* ── Hero Score Card ── */}
